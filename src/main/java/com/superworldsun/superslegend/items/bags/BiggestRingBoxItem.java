...
{
    public BiggestRingBoxItem(Properties properties)
    {
        super(properties);
    }
    ...
    public AbstractContainerMenu createContainer(int containerId, Inventory playerInventory, Player player, InteractionHand hand) {
        try {
            return new BiggestRingBoxContainer(containerId, playerInventory, hand);
        } catch (Exception e) {
            System.err.println("Error creating BiggestRingBoxContainer: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
