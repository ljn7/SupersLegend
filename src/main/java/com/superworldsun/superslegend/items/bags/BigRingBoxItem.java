...
{
    public BigRingBoxItem(Properties properties)
    {
        super(properties);
    }
    ...
    public AbstractContainerMenu createContainer(int containerId, Inventory playerInventory, Player player, InteractionHand hand) {
        try {
            return new BigRingBoxContainer(containerId, playerInventory, hand);
        } catch (Exception e) {
            System.err.println("Error creating BigRingBoxContainer: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
